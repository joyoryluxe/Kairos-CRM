...
export const getCorporateEvents = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (dateFrom || dateTo) {
      ...
      // Filter records to count for event-based fields
      const eventMatched = corporateEvents.filter(e => {
        if (!e.eventDateAndTime) return false;
        const eventDate = new Date(e.eventDateAndTime);
        return eventDate >= start && eventDate <= end;
      });

      // Calculate received amount from payments falling in the range
      let totalReceived = 0;
      corporateEvents.forEach(e => {
      ...
      summary = {
        total: eventMatched.length,
        totalRevenue: eventMatched.reduce((sum, e) => sum + (e.total || 0), 0),
        totalReceived,
        totalDue: eventMatched.reduce((sum, e) => sum + Math.max(e.balance || 0, 0), 0),
        totalExpenses: eventMatched.reduce((sum, e) => sum + (e.expenses || 0), 0),
        totalProfit: eventMatched.reduce((sum, e) => sum + (e.profit || 0), 0),
      };
    } else {
