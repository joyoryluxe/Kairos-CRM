...
export default function MaternityFormPage() {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    if (isEdit) {
      updateMutation.mutate({ payload: form as any });
    } else {
    ...
    <div className="animate-fade-up" style={{ maxWidth: 900, margin: "0 auto", padding: "0 1rem 3rem" }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Section title="Package" icon={<Package size={18} />}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
            <div>
              <label style={{ fontSize: "0.82rem", fontWeight: 600, display: "block", marginBottom: "0.35rem", color: "var(--text-secondary)" }}>Select Package</label>
              <select
                value={form.package}
            ...
            <div>
              <label style={{ fontSize: "0.82rem", fontWeight: 600, display: "block", marginBottom: "0.35rem", color: "var(--text-secondary)" }}>Package Price (auto)</label>
              <input
                readOnly
                value={packagePrice ? `₹${packagePrice.toLocaleString("en-IN")}` : "—"}
